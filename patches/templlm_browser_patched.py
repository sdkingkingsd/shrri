        ...
        try:
            while asyncio.get_event_loop().time() < deadline:
                ...
                if current_text.startswith(last_text) and len(current_text) > len(last_text):
                    ...
                    if delta.strip():
                        yield delta
                elif not current_text.startswith(last_text):
                    last_text = current_text if len(current_text) < len(last_text) else last_text

                if await self._is_generation_done(page):
                    final_text = await current_block.inner_text()
                    if final_text.startswith(last_text) and len(final_text) > len(last_text):
                        yield final_text[len(last_text):]
                    elif not final_text.startswith(last_text):
                        yield final_text
